...
<<iasql>>
  ...
  begin
    ...
    call create_load_balancer(
      iasql_engine_load_balancer, 'internet-facing', default_vpc, 'network', default_subnets, 'ipv4'
    );

    call create_listener(iasql_engine_load_balancer, iasql_engine_port, 'TCP', 'forward', iasql_engine_target_group);

    call create_load_balancer(
      iasql_postgres_load_balancer, 'internet-facing', default_vpc, 'network', default_subnets, 'ipv4'
    );

    call create_listener(iasql_postgres_load_balancer, iasql_postgres_port, 'TCP', 'forward', iasql_postgres_target_group);
