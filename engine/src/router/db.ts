    ...
    do {
      ...
      do {
        ...
        const promiseGenerators = records
          .filter(r => ['SecurityGroup', 'Instance', 'RDS', 'Repository', 'RepositoryPolicy', 'TaskDefinition', 'Cluster', 'Service', 'TargetGroup', 'ELB', 'Listener'].includes(r.table)) // TODO: Don't do this
          .map(r => {
            const name = r.table;
