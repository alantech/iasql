 ...
 * ```sql TheButton
 * SELECT iasql_begin();
 * INSERT INTO hosted_zone (domain_name) VALUES ('iasqlsample.com');
 *
 * INSERT INTO resource_record_set (name, record_type, record, ttl, parent_hosted_zone_id)
 * SELECT 'test.iasqlsample.com', 'CNAME', 'example.com.', 300, id FROM hosted_zone
 * WHERE domain_name = 'iasqlsample.com';
 * SELECT iasql_commit();
 * ```
 *
 * The `iasql_begin()` and `iasql_commit()` functions are IaSQL RPCs that are used to start and then end a transaction. We use those two functions to bundle changes to be pushed to the cloud immediately.
 * If you don't wrap the changes in a transaction, they'll be applied to the cloud in an eventually-consistent way.
 *
 ...
 * and [`resource_record_set`](https://dbdocs.io/iasql/iasql?table=resource_record_set&schema=public&view=table_structure) tables.
 *
 * ```sql TheButton
 * SELECT * FROM resource_record_set
