...
export class RegisteredInstanceMapper extends MapperBase<RegisteredInstance> {
  ...
  async registeredInstanceMapper(
    ...
    out.instance = await this.module.instance.db.read(ctx, registeredInstance.instanceId) ??
    ...
    out.targetGroup = await awsElbModule.targetGroup.db.read(
      ctx,
      registeredInstance.targetGroupArn
  ...
  async getRegisteredInstances(client: ElasticLoadBalancingV2) {
    ...
    const instanceTargetGroups = targetGroups
      .filter(tg => Object.is(tg.TargetType, TargetTypeEnum.INSTANCE)) ?? [];
    const out = [];
  ...
  cloud: Crud2<RegisteredInstance> = new Crud2({
    ...
    update: async (es: RegisteredInstance[], ctx: Context) => {
      ...
      const out = [];
      for (const e of es) {
        const cloudRecord = ctx?.memo?.cloud?.RegisteredInstance?.[
          this.module.registeredInstance.entityId(e)
        ];
        if (
          !e.instance?.instanceId ||
