...
export class TaskDefinitionMapper extends MapperBase<TaskDefinition> {
  ...
  equals = (a: TaskDefinition, b: TaskDefinition) => Object.is(a.cpuMemory, b.cpuMemory)
    ...
    && a.containerDefinitions
      .every(ac => !!b.containerDefinitions.find(bc => this.containersEq(ac, bc)));

  containersEq(a: ContainerDefinition, b: ContainerDefinition) {
    return Object.is(a.cpu, b.cpu)
      && Object.is(Object.keys(
        a.envVariables ?? {}).length,
        Object.keys(b.envVariables ?? {}
      ).length)
      && Object.keys(a.envVariables ?? {})
        .every((aevk: string) => !!Object.keys(b.envVariables ?? {})
          .find((bevk: string) => Object.is(aevk, bevk) &&
            Object.is(a.envVariables[aevk], b.envVariables[bevk])))
  ...
  createTaskDefinition = crudBuilderFormat<
    ECS,
  ...
  getTaskDefinition = crudBuilderFormat<
    ECS,
  ...
  async getTaskDefinitions(client: ECS) {
    ...
    const services = await this.module.service.getServices(
      client,
  ...
  async containerDefinitionMapper(c: any, ctx: Context) {
    ...
    if (containerImage?.includes('amazonaws.com')) {  // Private ECR
      ...
      try {
        const publicRepository = await awsEcrModule.publicRepository.db.read(
          ctx,
    ...
    if (c.logConfiguration?.logDriver === 'awslogs') {
      ...
      const logGroup = await awsCloudwatchModule.logGroup.db.read(ctx, groupName) ??
  ...
  async taskDefinitionMapper(td: any, ctx: Context) {
    ...
    out.cpuMemory = `vCPU${+(td.cpu ?? '256') / 1024}-${+(td.memory ?? '512') / 1024}GB` as
      CpuMemCombination;
  ...
  cloud = new Crud2({
    create: async (es: TaskDefinition[], ctx: Context) => {
      ...
      for (const e of es) {
        const containerDefinitions = e.containerDefinitions?.map(c => {
          ...
          if (
            c.envVariables &&
            Array.isArray(c.envVariables)
          ) throw new Error('Invalid environment variables format');
          container.environment = Object.keys(c.envVariables ?? {})
            .map((evk: string) => ({ name: evk, value: `${c.envVariables[evk]}`}));
        ...
        const newObject = await this.getTaskDefinition(
          client.ecsClient,
    ...
    read: async (ctx: Context, id?: string) => {
      ...
      if (id) {
        ...
        const taskDefs = ((await this.getTaskDefinitions(client.ecsClient)).taskDefinitions ?? [])
    ...
    delete: async (es: TaskDefinition[], ctx: Context) => {
      ...
      const esToDelete = [];
      for (const e of es) {
        if (Object.values(services)
          .find((s: any) => s.task?.taskDefinitionArn === e.taskDefinitionArn)
        ) {
      ...
      if (esWithServiceAttached.length) throw new Error(
        'Some tasks could not be deleted. They are attached to an existing service.'
      );
    },
  });
