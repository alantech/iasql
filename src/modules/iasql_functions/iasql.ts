...
export async function recreateQueries(
  ...
  // We need to recreate entities from changelogs in order to get the correct values in case the entity we
  // are trying to recreate is not in the database anymore
  const recreatedEntitiesFromChangelogs: EntitiesIndexesByEntityName = await recreateEntitiesFromChangelogs(
    changeLogs,
    modsIndexedByTable,
    orm,
  );
  // Now we handle each changelog to recreate the query
  for (const cl of changeLogs) {
    ...
    switch (cl.changeType) {
      case AuditLogChangeType.INSERT: {
        ...
        query = await getFormattedQuery(cl.tableName, AuditLogChangeType.INSERT, inverse, augmentedEntries);
        break;
      }
      case AuditLogChangeType.DELETE: {
        ...
        // Get formatted query
        query = await getFormattedQuery(cl.tableName, AuditLogChangeType.DELETE, inverse, augmentedEntries);
        break;
      }
...
/**
 * @internal
 * Return object including key, value and other metadata related to the value
 */
...
/**
 * @internal
 * Returns the formatted query to be used in the audit log based on the `type` and `inverse` parameters
 */
async function getFormattedQuery(
  ...
  switch (type) {
    ...
    case AuditLogChangeType.UPDATE: {
      // originalValues are the values that are going to be used in the WHERE clause
      // while updatedValues are the ones that are going to be used in the SET clause
...
/**
 * @internal
 * Returns the formatted pg value taking into account relevant metadata
 */
