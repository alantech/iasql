  ...
  switch (type) {
    case AuditLogChangeType.INSERT: {
      query = format(
        ...
        ...firstValues.map(e => e.formattedValue),
      );
      break;
    }
    case AuditLogChangeType.DELETE: {
      query = format(
        `
          ...
          WHERE ${firstValues
            ...
            .filter(e => e.key !== 'ami')
            .map(e => `${e.formattedValue === 'NULL' ? '%I IS %s' : e.isJson ? '%I::jsonb = %s' : '%I = %s'}`)
            .join(' AND ')};
        `,
        tableName,
        ...firstValues
          ...
          .flatMap(e => {
            if (e.isJson) {
              ...
              return [e.key, e.formattedValue.replace(jsonRegex, '::jsonb')];
            }
            return [e.key, e.formattedValue];
          }),
      );
      break;
    }
    ...
    case AuditLogChangeType.UPDATE: {
      ...
      `,
        tableName,
        ...(updatedValues?.flatMap(e => [e.key, e.formattedValue]) ?? []),
        ...(originalValues
          // We need to add an special case for AMIs since we know the revolve string can be used and it will not match with the actual AMI assigned
          ?.filter(e => e.key !== 'ami')
          ?.flatMap(e => {
...
async function augmentValue(
  ...
  if (value && metadata) {
    ...
    if (metadata instanceof EntityMetadata) {
      ...
      if (withRelationSubQueries && !!columnMetadata?.relationMetadata) {
        augmentedValue.formattedValue = await recreateSubQuery(
  ...
  if (!augmentedValue.formattedValue) {
    if (
      columnMetadata &&
      ...
      augmentedValue.formattedValue = Array.isArray(value)
        ? format('%L::%I', JSON.stringify(value), columnMetadata.type)
        : format('%L::%I', value, columnMetadata.type);
...
async function recreateSubQuery(
  ...
  if (cloudColumns && !(cloudColumns instanceof Error)) {
    ...
    if (values.every(v => v.formattedValue === 'NULL')) {
      dbColumns =
    ...
    const subQuery = format(
      ...
      ...columns.flatMap((c, i) => [snakeCase(c), values[i].formattedValue]),
    );
    return `(${subQuery})`;
  }
  return '';
