  ...
  for (const cl of changeLogs) {
    ...
    switch (cl.changeType) {
      case AuditLogChangeType.INSERT:
        // return values with primary db columns and the respective formatted values
        const primaryAugmentedValues = Object.entries(cl.change.change ?? {})
          .map(([k, v]: [string, any]) => augmentValue(k, v, tableMetadata))
          .filter(av => av.isPrimary);
        // create delete statement where the `WHERE` clause only takes into account db primary columns
        inverseQuery = format(
          `
            DELETE FROM %I
          ...
          `,
          cl.tableName,
          ...primaryAugmentedValues.flatMap(av => [av.key, av.formattedValue]),
        );
        break;
      case AuditLogChangeType.DELETE:
        // return values with all properties except db autogenerated columns and the respective formatted values
        const noGeneratedAugmentedValues = Object.entries(cl.change.original ?? {})
          .map(([k, v]: [string, any]) => augmentValue(k, v, tableMetadata))
          .filter(av => !av.isAutogenerated);
        // create insert statement
        inverseQuery = format(
          `
          INSERT INTO %I (${noGeneratedAugmentedValues.map(_ => '%I').join(', ')})
          VALUES (${noGeneratedAugmentedValues.map(_ => '%s').join(', ')});
        `,
          cl.tableName,
          ...noGeneratedAugmentedValues.map(av => av.key),
          ...noGeneratedAugmentedValues.map(av => av.formattedValue),
        );
        break;
      case AuditLogChangeType.UPDATE:
        // all original properties must be SET with the respective formatted values
        const originalAugmentedValues = Object.entries(cl.change.original ?? {}).map(
          ([k, v]: [string, any]) => augmentValue(k, v, tableMetadata),
        );
        // get from the changed properties only the primary db columns with the respective formatted values
        const changedAugmentedValues = Object.entries(cl.change.change ?? {})
          .map(([k, v]: [string, any]) => augmentValue(k, v, tableMetadata))
          .filter(av => av.isPrimary);
        // create update statement setting all properties and with a WHERE clause only using changed db primary columns
        inverseQuery = format(
          `
          UPDATE %I
