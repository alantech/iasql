...
const getTable = crudBuilder2<DynamoDB, 'describeTable'>(
  'describeTable',
  (TableName) => ({ TableName, }),
);
const listTables = paginateBuilder<DynamoDB>(paginateListTables, 'TableNames');
const createTable = crudBuilder2<DynamoDB, 'createTable'>(
  'createTable',
  (input) => input,
);
const updateTable = crudBuilder2<DynamoDB, 'updateTable'>(
  'updateTable',
  (input) => input,
);
const deleteTable = crudBuilder2<DynamoDB, 'deleteTable'>(
  'deleteTable',
  (TableName) => ({ TableName, }),
);
...
const dynamoMapper = (dynamo: TableDescription) => {
  ...
  out.throughput = dynamo.BillingModeSummary?.BillingMode === 'PAY_PER_REQUEST' ?
    'PAY_PER_REQUEST' : {
  ...
  out.primaryKey = Object.fromEntries(dynamo.KeySchema
    ?.sort((a, _b) => a.KeyType === 'HASH' ? -1 : 1)
...
export const AwsDynamoModule: Module2 = new Module2({
  ...
  mappers: {
    dynamoTable: new Mapper2<DynamoTable>({
      entity: DynamoTable,
      equals: (a: DynamoTable, b: DynamoTable) => isequal(
        ...
        create: async (es: DynamoTable[], ctx: Context) => {
          ...
          for (const e of es) {
            const req: CreateTableCommandInput = {
              ...
              AttributeDefinitions: Object.entries(e.primaryKey).map(
                ([AttributeName, AttributeType]) => ({ AttributeName, AttributeType, }),
              ),
              KeySchema: Object.entries(e.primaryKey).map(
                ([AttributeName, _], i) => ({
                  AttributeName,
            ...
            await waitUntilTableExists({
              ...
              minDelay: 1,
              maxDelay: 4,
            } as WaiterOptions<DynamoDB>, { TableName: e.tableName, });
