...
const attachRolePolicies = (client: IAM, roleName: string, policyArns: string[]) =>
  mapLin(policyArns, policyArn => attachRolePolicy(client, roleName, policyArn));
...
const detachRolePolicy = crudBuilder2<IAM, 'detachRolePolicy'>('detachRolePolicy', (RoleName, PolicyArn) => ({
  RoleName,
  PolicyArn,
}));

const detachRolePolicies = (client: IAM, roleName: string, policyArns: string[]) =>
  mapLin(policyArns, (policyArn: string) => detachRolePolicy(client, roleName, policyArn));

const deleteRole = crudBuilder2<IAM, 'deleteRole'>('deleteRole', RoleName => ({ RoleName }));
        ...
        cloud: new Crud2({
          create: async (es: Role[], ctx: Context) => {
            ...
            for (const role of es) {
              const roleArn = await createNewRole(client.iamClient, {
