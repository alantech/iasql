...
const getElasticIps = async (client: EC2) =>
  (await getAllIps(client))?.Addresses?.filter(a => !!a.AllocationId) ?? [];
const deleteElasticIp = crudBuilder2<EC2, 'releaseAddress'>('releaseAddress', AllocationId => ({
  AllocationId,
}));
  ...
  await createWaiter<EC2, DescribeNatGatewaysCommandInput>(
    ...
    async (cl, cmd) => {
      ...
      try {
        ...
        // If it is not a final state we retry
        if (
          [AwsNatGatewayState.DELETING, AwsNatGatewayState.PENDING].includes(out?.State as AwsNatGatewayState)
        ) {
          return { state: WaiterState.RETRY };
        }
        return { state: WaiterState.SUCCESS };
  ...
  await createWaiter<EC2, DescribeNatGatewaysCommandInput>(
    ...
    async (cl, cmd) => {
      ...
      try {
        ...
        // If it is not a final state we retry
        if (
          [AwsNatGatewayState.DELETING, AwsNatGatewayState.PENDING].includes(nat?.State as AwsNatGatewayState)
        ) {
          return { state: WaiterState.RETRY };
        }
        return { state: WaiterState.SUCCESS };
    ...
    utils: {
      subnetMapper: async (sn: AwsSubnet, ctx: Context) => {
        ...
        out.availabilityZone =
          (await AwsVpcModule.mappers.availabilityZone.db.read(ctx, sn.AvailabilityZone)) ??
          (await AwsVpcModule.mappers.availabilityZone.cloud.read(ctx, sn.AvailabilityZone));
        out.vpc =
          (await AwsVpcModule.mappers.vpc.db.read(ctx, sn.VpcId)) ??
      ...
      natGatewayMapper: async (nat: AwsNatGateway, ctx: Context) => {
        ...
        if (natPublicAddress) {
          out.elasticIp =
            (await AwsVpcModule.mappers.elasticIp.db.read(ctx, natPublicAddress.AllocationId)) ??
        ...
        out.subnet =
          (await AwsVpcModule.mappers.subnet.db.read(ctx, nat.SubnetId)) ??
      ...
      endpointGatewayMapper: async (eg: AwsVpcEndpoint, ctx: Context) => {
        ...
        out.vpc =
          (await AwsVpcModule.mappers.vpc.db.read(ctx, eg.VpcId)) ??
    ...
    mappers: {
      subnet: new Mapper2<Subnet>({
        ...
        equals: (a: Subnet, b: Subnet) =>
          Object.is(a.subnetId, b.subnetId) &&
      ...
      vpc: new Mapper2<Vpc>({
        ...
        cloud: new Crud2({
          updateOrReplace: (a: Vpc, b: Vpc) => {
            if (!Object.is(a.cidrBlock, b.cidrBlock) || !Object.is(a.isDefault, b.isDefault))
              return 'replace';
            else return 'update';
          },
          ...
          read: async (ctx: Context, id?: string) => {
            ...
            if (!!id) {
              ...
              return (await getVpcs(client.ec2client)).map(vpc => AwsVpcModule.utils.vpcMapper(vpc));
            }
          },
            ...
            for (const e of es) {
              ...
              if (!Object.is(e.state, cloudRecord.state)) {
                ...
                const isUpdate = Object.is(
                  AwsVpcModule.mappers.vpc.cloud.updateOrReplace(cloudRecord, e),
                  'update',
                );
                if (!isUpdate) {
      ...
      natGateway: new Mapper2<NatGateway>({
        ...
        cloud: new Crud2({
          ...
          updateOrReplace: (a: NatGateway, b: NatGateway) => {
            if (
              !(Object.is(a.state, b.state) && AwsVpcModule.utils.eqTags(a.tags, b.tags)) &&
              Object.is(a.connectivityType, b.connectivityType) &&
              Object.is(a.elasticIp?.allocationId, b.elasticIp?.allocationId) &&
      ...
      elasticIp: new Mapper2<ElasticIp>({
        entity: ElasticIp,
        equals: (a: ElasticIp, b: ElasticIp) =>
          Object.is(a.publicIp, b.publicIp) && AwsVpcModule.utils.eqTags(a.tags, b.tags),
        source: 'db',
        cloud: new Crud2({
        ...
        cloud: new Crud2({
          create: async (es: EndpointGateway[], ctx: Context) => {
            ...
            for (const e of es) {
              ...
              if (e.routeTableIds?.length) {
                ...
                input.RouteTableIds =
                  vpcRouteTables?.map(rt => rt.RouteTableId ?? '')?.filter(id => !!id) ?? [];
              }
              if (e.tags && Object.keys(e.tags).length) {
                const tags: Tag[] = Object.keys(e.tags).map((k: string) => {
                  return {
              ...
              const rawEndpointGateway = await getVpcEndpointGateway(
                client.ec2client,
                res?.VpcEndpointId ?? '',
              );
              const newEndpointGateway = await AwsVpcModule.utils.endpointGatewayMapper(
                rawEndpointGateway,
                ctx,
              );
          ...
          update: async (es: EndpointGateway[], ctx: Context) => {
            ...
            for (const e of es) {
              ...
              const isUpdate =
                AwsVpcModule.mappers.endpointGateway.cloud.updateOrReplace(cloudRecord, e) === 'update';
              if (isUpdate) {
                let update = false;
      ...
      availabilityZone: new Mapper2<AvailabilityZone>({
        ...
        cloud: new Crud2({
          create: async (e: AvailabilityZone[], ctx: Context) =>
            AwsVpcModule.mappers.availabilityZone.db.delete(e, ctx),
          ...
          delete: async (e: AvailabilityZone[], ctx: Context) =>
            AwsVpcModule.mappers.availabilityZone.db.create(e, ctx),
        }),
      }),
    },
  },
