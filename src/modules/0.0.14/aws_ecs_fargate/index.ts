...
const deleteClusterCore = crudBuilder2<ECS, 'deleteCluster'>('deleteCluster', input => input);
    ...
    utils: {
      ...
      containerDefinitionMapper: async (c: any, ctx: Context) => {
        ...
        if (containerImage?.includes('amazonaws.com')) {
          ...
          try {
            const repository =
              (await AwsEcrModule.mappers.repository.db.read(ctx, repositoryName)) ??
          ...
          try {
            const publicRepository =
              (await AwsEcrModule.mappers.publicRepository.db.read(ctx, publicRepositoryName)) ??
        ...
        if (c.logConfiguration?.logDriver === 'awslogs') {
          ...
          const logGroup =
            (await AwsCloudwatchModule.mappers.logGroup.db.read(ctx, groupName)) ??
            (await AwsCloudwatchModule.mappers.logGroup.cloud.read(ctx, groupName));
      ...
      taskDefinitionMapper: async (td: any, ctx: Context) => {
        ...
        out.cpuMemory = `vCPU${+(td.cpu ?? '256') / 1024}-${
          +(td.memory ?? '512') / 1024
      ...
      serviceMapper: async (s: any, ctx: Context) => {
        ...
        if (s.clusterArn) {
          out.cluster =
            (await AwsEcsFargateModule.mappers.cluster.db.read(ctx, s.clusterArn)) ??
            (await AwsEcsFargateModule.mappers.cluster.cloud.read(ctx, s.clusterArn));
        }
        out.desiredCount = s.desiredCount;
        const taskDefinition =
          (await AwsEcsFargateModule.mappers.taskDefinition.db.read(ctx, s.taskDefinition)) ??
        ...
        if (serviceLoadBalancer) {
          try {
            out.targetGroup =
              (await AwsElbModule.mappers.targetGroup.db.read(ctx, serviceLoadBalancer.targetGroupArn)) ??
        ...
        if (s.networkConfiguration?.awsvpcConfiguration) {
          ...
          for (const sn of networkConf.subnets ?? []) {
            ...
            try {
              subnet =
                (await AwsVpcModule.mappers.subnet.db.read(ctx, sn)) ??
        ...
        cloud: new Crud2({
          ...
          update: async (es: Cluster[], ctx: Context) => {
            ...
            for (const e of es) {
              ...
              const isUpdate =
                AwsEcsFargateModule.mappers.cluster.cloud.updateOrReplace(cloudRecord, e) === 'update';
              if (isUpdate) {
      ...
      taskDefinition: new Mapper2<TaskDefinition>({
        ...
        cloud: new Crud2({
          ...
          delete: async (es: TaskDefinition[], ctx: Context) => {
            ...
            for (const e of es) {
              if (
                Object.values(services).find((s: any) => s.task?.taskDefinitionArn === e.taskDefinitionArn)
              ) {
        ...
        cloud: new Crud2({
          ...
          update: async (es: Service[], ctx: Context) => {
            ...
            for (const e of es) {
              ...
              const isUpdate =
                AwsEcsFargateModule.mappers.service.cloud.updateOrReplace(cloudRecord, e) === 'update';
              if (isUpdate) {
