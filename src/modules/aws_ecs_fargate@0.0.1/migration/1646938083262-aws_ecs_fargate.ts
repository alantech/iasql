...
export class awsEcsFargate1646938083262 implements MigrationInterface {
    name = 'awsEcsFargate1646938083262'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "aws_container_definition" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "image" character varying, "tag" character varying, "digest" character varying, "essential" boolean NOT NULL DEFAULT false, "cpu" integer, "memory" integer, "memory_reservation" integer, "host_port" integer, "container_port" integer, "protocol" "public"."aws_container_definition_protocol_enum", "env_variables" text, "task_definition_id" integer, "repository_id" integer, "public_repository_id" integer, "log_group_id" integer, CONSTRAINT "CHK_3a97fff3cafc39ce686f5b2a08" CHECK (("image" is null and ("tag" is not null or "digest" is not null)) or "image" is not null), CONSTRAINT "CHK_84faf9d3827126ceebee8a6117" CHECK (("image" is null and ("repository_id" is not null or "public_repository_id" is not null)) or "image" is not null), CONSTRAINT "PK_82905170a50ef6bbf6931d799a0" PRIMARY KEY ("id"))`);
