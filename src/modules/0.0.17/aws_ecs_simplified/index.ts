...
class EcsSimplifiedMapper extends MapperBase<EcsSimplified> {
  ...
  getSimplifiedObjectMapped(e: EcsSimplified) {
    ...
    const containerDefinition = this.simplifiedEntityMapper.containerDefinition(prefix, e.appName, e.appPort,
      e.cpuMem, taskDefinition, logGroup, e.imageTag, e.imageDigest);
    const service = this.simplifiedEntityMapper.service(prefix, e.appName, e.desiredCount, e.publicIp ?? false, cluster,
    ...
    create: async (es: EcsSimplified[], ctx: Context) => {
      ...
      for (const e of es) {
        ...
        try {
          ...
          await this.cloudFns.create.loadBalancer(client, simplifiedObjectMapped.loadBalancer, defaultSubnets);
          ...
          step = 'createCluster';
          // task with container
          await this.cloudFns.create.taskDefinition(client, simplifiedObjectMapped.taskDefinition, simplifiedObjectMapped.containerDefinition, simplifiedObjectMapped.repository);
          step = 'createTaskDefinition';
          // service and serv sg
          await this.cloudFns.create.service(client, simplifiedObjectMapped.service, simplifiedObjectMapped.containerDefinition, defaultSubnets);
          step = 'createService';
          ...
          try {
            switch (step) {
              ...
              case 'createSecurityGroupRules':
                await this.cloudFns.delete.securityGroupRules(client, simplifiedObjectMapped.securityGroupRules);
