...
class DynamoTableMapper extends MapperBase<DynamoTable> {
  ...
  getTable = crudBuilder2<DynamoDB, 'describeTable'>(
    'describeTable',
    (TableName) => ({ TableName, }),
  );
  listTables = paginateBuilder<DynamoDB>(paginateListTables, 'TableNames');
  createTable = crudBuilder2<DynamoDB, 'createTable'>(
    'createTable',
    (input) => input,
  );
  updateTable = crudBuilder2<DynamoDB, 'updateTable'>(
    'updateTable',
    (input) => input,
  );
  deleteTable = crudBuilder2<DynamoDB, 'deleteTable'>(
    'deleteTable',
    (TableName) => ({ TableName, }),
  );
