...
const getListener = crudBuilderFormat<
  ElasticLoadBalancingV2,
  'describeListeners',
  ListenerAws | undefined
>(
...
const getListeners = async (client: ElasticLoadBalancingV2, loadBalancerArns: string[]) => (
  await mapLin(loadBalancerArns, getListenersForArn.bind(null, client))
).flat();
const updateListener = crudBuilderFormat<
  ElasticLoadBalancingV2,
  'modifyListener',
  ListenerAws | undefined
>(
          ...
          for (const e of es) {
            ...
            if (isUpdate) {
              ...
              if (!Object.is(cloudRecord.ipAddressType, e.ipAddressType)) {
                ...
                const updatedLoadBalancer = await getLoadBalancer(client.elbClient, e.loadBalancerArn ?? '');
