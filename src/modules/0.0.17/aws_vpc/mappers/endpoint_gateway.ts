...
export class EndpointGatewayMapper extends MapperBase<EndpointGateway> {
  ...
  async endpointGatewayMapper(eg: AwsVpcEndpoint, ctx: Context) {
    ...
    out.service = service;
    out.vpc = await this.module.vpc.db.read(ctx, eg.VpcId) ??
      await this.module.vpc.cloud.read(ctx, eg.VpcId);
  ...
  createVpcEndpointGateway = crudBuilderFormat<
    EC2,
    'createVpcEndpoint',
  ...
  getVpcEndpointGateway = crudBuilderFormat<
    EC2,
  ...
  deleteVpcEndpointGateway = crudBuilderFormat<
    EC2,
    'deleteVpcEndpoints',
      ...
      for (const e of es) {
        ...
        if (isUpdate) {
          ...
          if (!(Object.is(cloudRecord.routeTableIds?.length, e.routeTableIds?.length)
              && !!cloudRecord.routeTableIds
                ?.every((crrt: any) => !!e.routeTableIds?.find(ert => Object.is(crrt, ert))))) {
