...
export class SubnetMapper extends MapperBase<Subnet> {
  ...
  async subnetMapper(sn: AwsSubnet, ctx: Context) {
    ...
    out.availabilityZone = await this.module.availabilityZone.db.read(ctx, sn.AvailabilityZone) ??
      await this.module.availabilityZone.cloud.read(ctx, sn.AvailabilityZone);
    out.vpc = await this.module.vpc.db.read(ctx, sn.VpcId) ??
      await this.module.vpc.cloud.read(ctx, sn.VpcId);
