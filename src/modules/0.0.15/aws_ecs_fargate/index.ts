...
const getClusters = async (client: ECS) => getClustersCore(client, { clusters: await getClusterArns(client) });
      ...
      taskDefinitionMapper: async (td: any, ctx: Context) => {
        ...
        if (td.executionRoleArn) {
          ...
          if (!Object.values(ctx.memo?.cloud?.Role ?? {}).length) {
            ...
            out.executionRole =
              (await AwsIamModule.mappers.role.db.read(ctx, roleName)) ?? ctx?.memo?.cloud?.Role?.[roleName ?? ''];
          }
        }
        ...
        cloud: new Crud2({
          ...
          read: async (ctx: Context, id?: string) => {
            ...
            if (id) {
              ...
              const taskDefs = ((await getTaskDefinitions(client.ecsClient)).taskDefinitions ?? []).filter(td =>
