...
SELECT _ FROM iasql_install('aws_ec2');
SELECT _ FROM iasql_install('aws_ec2_metadata');
...
INSERT INTO instance (id, ami, instance_type, subnet_id, tags, user_data, region)
SELECT 2, 'resolve:ssm:/aws/service/canonical/ubuntu/server/22.04/stable/current/amd64/hvm/ebs-gp2/ami-id', 't2.small', subnet.id, '{"name":"ghost-blog"}', '#!/bin/bash
useradd -m ghostuser
...
(SELECT id FROM security_group WHERE group_name='ghost_sg_group' AND region = 'us-east-2' limit 1);
