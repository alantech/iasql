---
slug: deploy-ghost-ec2-production
title: Deploy Ghost in EC2 at production level
date: 2022-11-25
authors: [yrobla]
---

# Deploy <a href="https://ghost.org/">Ghost</a> at production level

On this previous <a href="https://iasql.com/blog/deploy-ghost-ec2/">article</a> we installed Ghost in EC2 with one click, for testing and development purposes. Once it is ready to be deployed in production,
there are several steps needed to be able to show to end customers and scale as needed.

### Step 1: Add a custom hostname

The default installation was using the public IPV4 of the instance to serve Ghost content. For production a custom hostname is needed. This can be achieved by creating a hosted zone in AWS, and creating an
A record pointing to the EC2 public ip:
...
INSERT INTO
  resource_record_set (NAME, record_type, ttl, parent_hosted_zone_id, record)
SELECT
  'ghost.test-ghost.com.',
  'A',
  900,
  id,
  (
