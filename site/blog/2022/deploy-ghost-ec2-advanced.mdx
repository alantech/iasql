...
```sql TheButton[Generate A record for domain]="Generate A record for domain"
 -- Install modules
SELECT * FROM iasql_install ('aws_route53', 'aws_ec2_metadata');

-- Grab public IP for previously created instance
SELECT public_ip_address FROM instance_metadata WHERE id = (
    SELECT id FROM instance WHERE tags ->> 'name' = 'ghost-blog' LIMIT 1
  );
...
After some period of time, and assuming that the domain nameservers are pointing to <a href="https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/migrate-dns-domain-in-use.html#migrate-dns-change-name-servers-with-provider">AWS nameservers</a>,
