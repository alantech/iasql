...
```sql TheButton[Delete old recordset]="Delete old recordset"
select * from iasql_begin();
...
select * from iasql_begin();
insert into alias_target (evaluate_target_health, load_balancer_id) select true, id from load_balancer where load_balancer_name='ghost' limit 1;
insert into resource_record_set (name, record_type, alias_target_id, parent_hosted_zone_id) values ('blog.test-ghost.com.', 'A',
(select id FROM alias_target WHERE load_balancer_id=(SELECT id FROM load_balancer WHERE load_balancer_name='ghost') LIMIT 1),
(select id from hosted_zone where domain_name='test-ghost.com.' limit 1));
select * from iasql_commit();
```
...
The current approach was using an internal mySQL database hosted on the same EC2 instance. This could lead to unstability if the database goes
down and will be a problem for scaling if several EC2 instances are needed. A more recommendable approach could be to use an externally
managed AWS database. We can rely on <a href="https://aws.amazon.com/rds/">AWS RDS</a> for it.

```sql TheButton[Create database]="Create database"
select * from iasql_begin();
INSERT INTO security_group (description, group_name) VALUES ('RDS security Group', 'rds');

INSERT INTO security_group_rule (is_egress, ip_protocol, from_port, to_port, cidr_ipv4, description, security_group_id)
SELECT false, 'tcp', 3306, 3306, '0.0.0.0/0', 'rds_mysql', id FROM security_group WHERE group_name = 'rds';

insert into rds (db_instance_identifier, allocated_storage, db_instance_class, engine, master_user_password, master_username, availability_zone) values
('ghost', 20, 'db.t3.micro', 'mysql', 'admin2022', 'admin', 'us-east-2a');

INSERT INTO rds_security_groups (rds_id, security_group_id) SELECT
  (SELECT id FROM rds WHERE db_instance_identifier='ghost'),
  (SELECT id FROM security_group WHERE group_name='rds' AND region = 'us-east-2');
...
```sql TheButton[Retrieve database details]="Retrieve database details"
select endpoint_addr, endpoint_port from rds;
```
