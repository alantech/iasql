...
CodePipeline is a service provided by AWS to provide a fully automated [Continuous Delivery](https://en.wikipedia.org/wiki/Continuous_delivery) service.
It allows you to fully manage all the steps in the code delivery lifecycle, from checking out the source code, building it, testing it, and deploying to production.
...
The indications for building aThe instructions for building and deploying the application are defined in the [`appspec.yml`](https://github.com/iasql/iasql-code-example-node/blob/main/appspec.yml) file.
This file contains all the information needed by CodeDeploy to manage the whole lifecycle of the application (Complete reference can be seen
