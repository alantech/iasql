...
That's all folks. After installing `aws_ecs_fargate` module, which installs the dependant `aws_ecr` module, the schema relations between the AWS ECS service will not let you delete ECR images currently in use and will reinstate the ones that are currently active. However, if you would like to delete images that are untagged, and unused, you can use the following query:
