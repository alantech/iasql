---
slug: clean-your-aws-account
title: Clean your AWS account with one click
date: 2022-10-27
authors: [yrobla]
---

# Clean your AWS account by simply clicking a button

One of the main complexities of working with AWS is the number of resources that can be created, either by manual or automated workflows. The chained resources and their dependencies can quickly get out of control,
and it could be a nightmare to clean up then.

This is a common problem when using tests accounts, either for developing or testing some integrations. Often these accounts have resources out of control, causing you to incur on extra costs, or causing unexpected conflicts.
Cleaning those manually can be a hard task... manually visiting each resource, investigating the dependencies, manually cleaning... it is a time-consuming, error-prone task. If we consider that those resources may be created in
multiple regions, the manual procedure can become a very difficult task.

To solve this problem you can use IaSQL. By simply clicking a button, IaSQL will get a snapshot of the status of your cloud, and will schedule deletion of the existing resources and their dependencies until your account is completely cleaned.

**Want to try? Connect your AWS account to [IaSQL](https://iasql.com) for free. Read more about AWS account management in this part of our [docs](https://iasql.com/docs/aws/).**

:::info

Please note that IaSQL can offer this cleanup only for the resources covered by our system. You can get a full coverage list <a href="https://github.com/iasql/iasql#aws-services-with-significant-api-coverage">here</a>.

:::info


```sql TheButton[AWS account cleanup]="Clean your AWS Account"
-- Install all IaSQL modules
SELECT iasql_install(variadic array(select module_name from iasql_modules_list()));
SELECT iasql_begin();
-- Delete all resources from the db
SELECT * FROM delete_all_records();
-- See the changes that are going to be applied
SELECT iasql_preview();
-- Rollback the changes
SELECT iasql_rollback();
-- To apply the changes please uncomment the following line (and comment the rollback line) to perform the cleanup
-- SELECT iasql_commit();
```

After the initial module install, IaSQL will inspect your cloud and propose the resources to delete.
If you agree with the proposed changes, please uncomment the apply statement and execute it, to let IaSQL completely cleans up your account.

<img src="/img/clean_all_screenshot.png" style={{ maxWidth: 800 }} />
