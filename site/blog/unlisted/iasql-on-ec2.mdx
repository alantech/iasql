...
To know how the above functions work, and why they exist, we should dive into the internals of IaSQL. IaSQL leverages [`pg_cron`](https://github.com/citusdata/pg_cron) Postgres extension to compare the state of the cloud with the database. Approximately every two minutes, IaSQL is triggered to check the state of the database, state of the cloud, and apply the needed changes whether on cloud or on the database. So any changes to the database will be "queued" until the next cron tick comes and applies them.

But what if we need our changes to be pushed to the cloud immediately and not wait for that every-2-minutes cron tick? In that case we can surround our database modifications with a pair of `iasql_begin` and `iasql_commit`. In a nutshell, what they do is as follows:
- `iasql_begin`: It'll stop the default cron behavior. therefore your changes won't be pushed to the cloud until you manually execute `iasql_commit`. This can be used to "batch" the changes and maybe review them (using `iasql_preview`) before applying.
- `iasql_commit`: Immediately apply the changes from the database, and sync the state of the cloud.