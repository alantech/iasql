...
Did you know you can deploy a static website using a SQL REPL? In this post, we'll show you how to use IaSQL to deploy a static website from your GitHub repository to AWS using SQL commands.
...
Installing a module gives us ability to use tables and RPCs provided by it. `aws_s3` module gives us the ability to manage an S3 bucket, S3 static website hosting, and other related stuff. So let's create an S3 bucket first.
...
The above query will have IaSQL to create a new bucket in the `us-east-1` region with the defined name `<bucket-name>` and the given policy. The `iasql_begin` and `iasql_commit` functions are RPCs that will start and finish an IaSQL transaction. Learn more about IaSQL transactions in this part of our [documentation](/docs/transaction/).
