...
```sql TheButton[Query for missing endpoints]="Query for missing endpoints"
...
to control traffic.
<a href="https://docs.aws.amazon.com/IAM/latest/UserGuide/access_controlling.html">IAM policies</a>can also be added to control access to those endpoints.
...
```sql TheButton[Insert missing endpoints]="Insert missing endpoints"
...
```sql TheButton[Insert missing Gateway endpoints]="Insert missing Gateway endpoints"
 -- Inserts the missing endpoints
SELECT * FROM iasql_begin();
