...
<p>Are you using S3 buckets as part of your cloud deployments ? How are you accessing them?</p>
<p>
  When not configured properly, access to the S3 buckets may need internet access. One simple but costly way
  is to rely on{' '}
  <a href="https://docs.aws.amazon.com/en_en/vpc/latest/userguide/vpc-nat-gateway.html" target="_blank">
    NAT gateways
  </a>
...
<p>
  When the VPC endpoints are enabled you could access your S3 buckets using this endpoint. This will allow you
...
The two main reasons are **security** and **performance**.
...
<p>PrivateLink consistently lowers the amount of errors and timeouts on high loads</p>
...
<p>
  And endpoint interface is an elastic network interface with a private IP address that serves as an entry
...
<p>In order to use them, DNS resolution must be enabled in the VPC</p>
<p>
...
<p>Gateway endpoints only support S3 and DynamoDB services.</p>

<p>
  <a href="https://app.iasql.com/#/button/Add S3 endpoint interfaces/--%20Inserts%20the%20missing%20endpoints%0Ainsert%20into%20endpoint_gateway%28region%2C%20vpc_id%2C%20service%29%20select%20bucket.region%2C%20vpc.id%2C%20%27s3%27%20from%20bucket%20inner%20join%20vpc%20on%20bucket.region%3Dvpc.region%20where%20not%20exists%20%28select%20id%20from%20endpoint_gateway%20where%20endpoint_gateway.region%3Dbucket.region%20and%20endpoint_gateway.vpc_id%3Dvpc.id%29%0A--%20Preview%20the%20changes%0Aselect%20%2A%20from%20iasql_preview_apply%28%29%3B%0A--%20Apply%20the%20changes%0A--select%20%2A%20from%20iasql_apply%28%29%3B">
...
<p>
  ...
  preview the changes to be applied on your cloud.
</p>
...
<div class="col col--12">
  <img src="/img/ec2_using_vpc.png" height="200" />
</div>

<p>For endpoint interfaces, access also can be tested using the named endpoint:</p>

<img src="/img/s3_via_interface.png" />
