...
pub async fn main() {
  ...
  let app = App::new(crate_name!())
    ...
    .subcommands(vec![
      SubCommand::with_name("login")
        ...
        .arg(Arg::with_name("noninteractive").takes_value(true)),
  ...
  let noninteractive = matches.is_present("noninteractive");
  if let Some("logout") = matches.subcommand_name() {
    return auth::logout(noninteractive);
  } else if let Some("login") = matches.subcommand_name() {
    return auth::login(true, noninteractive).await;
  } else {
  ...
  match matches.subcommand() {
    ("new", Some(s_matches)) => {
      ...
      db::new(&db, s_matches.is_present("noninteractive")).await
    }
    ("import", Some(s_matches)) => {
      let db = db::get_or_input_db(s_matches.value_of("db")).await;
      let dump_file = db::get_or_input_arg(s_matches.value_of("dump_file"), "Dump file");
      db::import(&db, &dump_file, s_matches.is_present("noninteractive")).await
    }
    ...
    ("remove", Some(s_matches)) => {
      let db = db::get_or_select_db(s_matches.value_of("db")).await;
      db::remove(&db, s_matches.is_present("noninteractive")).await
    }
    ("apply", Some(s_matches)) => {
      let db = db::get_or_select_db(s_matches.value_of("db")).await;
      db::apply(&db, s_matches.is_present("noninteractive")).await
    }
    ("plan", Some(s_matches)) => {
      let db = db::get_or_select_db(s_matches.value_of("db")).await;
      db::plan(&db, s_matches.is_present("noninteractive")).await
    }
    ("sync", Some(s_matches)) => {
      let db = db::get_or_select_db(s_matches.value_of("db")).await;
      db::sync(&db, s_matches.is_present("noninteractive")).await
    }
    ("dbs", _) => {
      db::list().await;
    }
    ...
    ("install", Some(s_matches)) => {
      ...
      module::install(&db, modules, s_matches.is_present("noninteractive")).await;
    }
    ("uninstall", Some(s_matches)) => {
      let db = db::get_or_select_db(s_matches.value_of("db")).await;
      let modules = module::mods_to_remove(&db, s_matches.values_of_lossy("modules")).await;
      module::uninstall(&db, modules, s_matches.is_present("noninteractive")).await;
    }
