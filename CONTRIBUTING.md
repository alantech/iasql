...
Go to the Actions tab and click the ["Version: Release Newest" tab](https://github.com/iasql/iasql/actions/workflows/version.yml). Then click the "Run Workflow" button to mark the newest version as released.

### 2. Begin development of the next version

Go to the Actions tab and click the ["Version: Start New Development" tab](https://github.com/iasql/iasql/actions/workflows/develop.yml). Next click the "Run Workflow" button. Then type in the new version number for the next release and finally click the green "Run workflow" button. It will create a commit with that version with a `-beta` appended to it to make it clear this and following commits are not for users to work with directly.
