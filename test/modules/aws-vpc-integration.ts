...
`,
  ...
  itDocs(
    ...
    query(
      `
      UPDATE vpc SET dhcp_options_id = (SELECT id FROM dhcp_options WHERE tags ->> 'name' = '${prefix}-1') WHERE tags ->> 'name' = '${prefix}-peering-vpc';;
  `,
      undefined,
      true,
      () => ({ username, password }),
  ...
  itDocs(
    ...
    query(
      `
      SELECT dhcp_options_id FROM vpc WHERE tags ->> 'name' = '${prefix}-peering-vpc' AND dhcp_options_id = (SELECT id FROM dhcp_options WHERE tags ->> 'name' = '${prefix}-1');
  `,
      (res: any) => expect(res.length).toBe(1),
    ),
  );
  ...
  itDocs(
    ...
    query(
      `
      UPDATE vpc SET dhcp_options_id = 'default' WHERE tags ->> 'name' = '${prefix}-peering-vpc';;
  `,
      undefined,
      true,
      () => ({ username, password }),
