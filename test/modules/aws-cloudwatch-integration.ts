...
describe('AwsCloudwatch Integration Testing', () => {
  ...
  it('creates a log group in default region', query(`
    INSERT INTO log_group (log_group_name)
    VALUES ('${logGroupName}');
  `));

  it('also creates a log group in non-default region with the same name', query(`
  ...
  it('ARNs and regions for the two log groups with the same name should be different', query(`
    SELECT *
    FROM log_group;
  `, (res: any) => {
    // two log groups
  ...
  it('deletes the log group from the non-default region', query(`
    DELETE FROM log_group
  ...
  it('checks if the log group from the non-default region is back', query(`
    SELECT * FROM log_group
    WHERE log_group_name = '${logGroupName}' AND region != default_aws_region();
  `, (res: any) => expect(res.length).toBe(1)));

  it('deletes the log group from the non-default region, this time for real', query(`
    DELETE FROM log_group
    WHERE log_group_name = '${logGroupName}' AND region != default_aws_region();
  `));
  ...
  it('checks if the log group in the default region is still there', query(`
    SELECT * FROM log_group
    WHERE log_group_name = '${logGroupName}' AND region = default_aws_region();
  `, (res: any) => expect(res.length).toBe(1)));

  it('checks if the log group in the non-default region is gone', query(`
    SELECT * FROM log_group
    WHERE log_group_name = '${logGroupName}' AND region != default_aws_region();
  `, (res: any) => expect(res.length).toBe(0)));

  it('deletes the log group in the default region', query(`
    DELETE FROM log_group
    WHERE log_group_name = '${logGroupName}' AND region = default_aws_region();
  `));
  ...
  it('checks the deletion of all of the log groups', query(`
    SELECT *
    FROM log_group
    WHERE log_group_name = '${logGroupName}';
  `, (res: any[]) => expect(res.length).toBe(0)));

  it('creates a log group to be moved to another region', query(`
    INSERT INTO log_group (log_group_name, region)
  ...
  it('moves the log group to a new region', query(`
    UPDATE log_group
  ...
  it('checks if the log group has been moved to the new region', query(`
    SELECT * FROM log_group
    WHERE log_group_name = '${logGroupName}';
  `, (res: any) => {
  ...
  it('deletes all the log groups for the last time', query(`
    DELETE FROM log_group;
  `));
