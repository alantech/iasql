...
`));
  ...
  it('adds a new service', query(`
    BEGIN;
      ...
      VALUES ('${serviceRepositoryName}', ${serviceDesiredCount}, (select array(select subnet.id from subnet inner join vpc on vpc.id = subnet.vpc_id where is_default = true limit 3)), 'ENABLED', '${clusterName}', (select id from task_definition where family = '${tdRepositoryFamily}' order by revision desc limit 1), '${serviceTargetGroupName}');
